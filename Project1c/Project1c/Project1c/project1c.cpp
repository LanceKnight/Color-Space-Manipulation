...
double* RGB2Luv(int r, int g, int b){

	//cout << "r= " << r<<"\n";
	//from pixel to nonlinear RGB
	double nonlinear_r = ((double)r) / 255;
	double nonlinear_g = ((double)g) / 255;
	double nonlinear_b = ((double)b) / 255;
...
int* Luv2RGB(int L, int u, int v){
	...
	double u_p = (u + (double)13 * uw * (double)L) / ((double)13 * (double)(L+1));
	double v_p = (v + (double)13 * vw*(double)L) / ((double)13 * (double)(L+1));
	/*cout << "u_p = " << u_p << "\n";
	cout << "v_p = " << v_p << "\n";
	cout << "L inside:" << L << "\n";*/
	double Y = 0;
...
void runOnWindow(int W1, int H1, int W2, int H2, Mat inputImage, char *outName) {
	...
	//cout << (H2 - H1+1)*(W2 - W1+1)<<"\n";
	for (int t = 0; t < 101; t++)
		cout << "t=" << t<<", hist:"<<hist[t]<<", f[t]:"<<f[t]<<"\n";
	cout << "min= " << min << "\n";
	cout << "max= " << max << "\n";
	for (int i = 0; i < rows; i++)
		for (int j = 0; j < cols; j++){
			...
			/*cout << "L= " << L <<"\n";
			cout << "f(L-1)" << f[(int)L - 1] << ", f(L)" << f[(int)L]<<"\n";
			cout << "num of pixels" << (H2 - H1 + 1)*(W2 - W1 + 1) << "\n";*/
			//cout << "u= " << u << "\n";
			//cout << "v= " << v << "\n";
			//int new_L[101];
			//int idx = 0;
			if (L != 0){
				L = floor(((double)(f[(int)L - 1] + f[(int)L]) *(double)101) / ((double)2 * (double)((H2 - H1 + 1)*(W2 - W1 + 1))));
				if (L == 101){
					L = 100;
				}
				//new_L[idx] = L;
				...
				L = ((double) f[(int)L] *(double)101) / ((double)2 * (double)((H2 - H1 + 1)*(W2 - W1 + 1)));
				if (L == 101){
					L = 100;
				}
			}
